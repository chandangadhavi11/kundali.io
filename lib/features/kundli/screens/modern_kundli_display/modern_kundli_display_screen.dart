...
class ModernKundliDisplayScreen extends StatefulWidget {
  final KundaliData kundaliData;

  const ModernKundliDisplayScreen({super.key, required this.kundaliData});

  @override
  State<ModernKundliDisplayScreen> createState() =>
      _ModernKundliDisplayScreenState();
}

class _ModernKundliDisplayScreenState extends State<ModernKundliDisplayScreen>
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          if (widget.kundaliData.isPrimary) ...[
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 4),
                          Text(
                            _formatDateTime(widget.kundaliData.birthDateTime),
                            style: TextStyle(
                              fontSize: 12,
                              color: isDarkMode ? Colors.grey[400] : Colors.grey[600],
                            ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
                ...
                child: CustomPaint(
                  ...
                  painter: _currentChartStyle == ChartStyle.northIndian
                      ? NorthIndianChartPainter(
                          houses: widget.kundaliData.houses,
                          planets: widget.kundaliData.planetPositions,
          ...
          // Yogas and Doshas
          if (widget.kundaliData.yogas.isNotEmpty ||
              widget.kundaliData.doshas.isNotEmpty)
  ...
  Widget _buildCompactInfoCards(bool isDarkMode) {
    final cards = [
      {
        'title': 'Asc',
        'value': widget.kundaliData.ascendant.formattedPosition,
        'icon': CupertinoIcons.arrow_up_circle_fill,
        'color': const Color(0xFF6B4EE6),
      },
      {
        'title': 'Moon',
        'value': widget.kundaliData.moonSign,
        'icon': CupertinoIcons.moon_fill,
        'color': const Color(0xFF4ECDC4),
      },
      {
        'title': 'Sun',
        'value': widget.kundaliData.sunSign,
        'icon': CupertinoIcons.sun_max_fill,
        'color': const Color(0xFFFFB347),
      },
      {
        'title': 'Naksh',
        'value': widget.kundaliData.birthNakshatra,
      ...
      children: [
        if (widget.kundaliData.yogas.isNotEmpty) ...[
          ...
          Wrap(
            spacing: 6,
            runSpacing: 6,
            children: widget.kundaliData.yogas.map((yoga) {
              return Container(
        ...
        if (widget.kundaliData.doshas.isNotEmpty) ...[
          Text(
            'Doshas Present',
            style: TextStyle(
              fontSize: 15,
          ...
          Wrap(
            spacing: 6,
            runSpacing: 6,
            children: widget.kundaliData.doshas.map((dosha) {
              return Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                decoration: BoxDecoration(
  ...
  Widget _buildPlanetsTab(bool isDarkMode) {
    return ListView.builder(
      ...
      itemCount: widget.kundaliData.planetPositions.length,
      itemBuilder: (context, index) {
        final planet = widget.kundaliData.planetPositions.values.elementAt(index);
  ...
  Widget _buildHousesTab(bool isDarkMode) {
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: widget.kundaliData.houses.length,
      itemBuilder: (context, index) {
        final house = widget.kundaliData.houses[index];
  ...
  Widget _buildDashaTab(bool isDarkMode) {
    final dashaInfo = widget.kundaliData.dashaInfo;
      ...
      child: Column(
        ...
        children: [
          _buildCompactReportSection(
            ...
            content: 'Based on your ascendant ${widget.kundaliData.ascendant.sign} and Moon sign ${widget.kundaliData.moonSign}, you possess a unique blend of qualities...',
  ...
  void _handleMenuAction(String action) {
    switch (action) {
      case 'export':
        ...
        break;
      case 'primary':
        context.read<KundliProvider>().setPrimaryKundali(widget.kundaliData.id);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
