...
class _InteractiveKundliChartState extends State<InteractiveKundliChart>
  ...
  void _buildHousePaths(Size size) {
    ...
    // Houses go CLOCKWISE starting from House 1 at top center
    // ═══════════════════════════════════════════════════════════════

    // House 1 - TOP CENTER KITE (Ascendant house)
    // Vertices: MT (top midpoint) → P1 → C (center) → P2 → back to MT
    _housePaths[0] = Path()
      ..moveTo(MT.dx, MT.dy)
      ..lineTo(P1.dx, P1.dy)
      ..lineTo(C.dx, C.dy)
    ...
    // Vertices: MR → P2 → C → P3 → back to MR
    _housePaths[3] = Path()
      ..moveTo(MR.dx, MR.dy)
      ..lineTo(P2.dx, P2.dy)
      ..lineTo(C.dx, C.dy)
    ...
    // Vertices: MB → P3 → C → P4 → back to MB
    _housePaths[6] = Path()
      ..moveTo(MB.dx, MB.dy)
      ..lineTo(P3.dx, P3.dy)
      ..lineTo(C.dx, C.dy)
    ...
    // Vertices: ML → P4 → C → P1 → back to ML
    _housePaths[9] = Path()
      ..moveTo(ML.dx, ML.dy)
      ..lineTo(P4.dx, P4.dy)
      ..lineTo(C.dx, C.dy)
  ...
  int? _getHouseAtPosition(Offset position) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GestureDetector(
          onTapDown: (details) {
            _buildHousePaths(size);
            final house = _getHouseAtPosition(details.localPosition);
            if (house != null) {
              setState(() => _pressedHouse = house);
          ...
          onPanEnd: (_) {
            if (_selectedHouse != null) {
              _showHouseDetails(context, _selectedHouse!);
            }
...
class _GlowingChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paths = housePaths.isNotEmpty ? housePaths : _buildLocalPaths(
    ...
    for (int i = 0; i < 12; i++) {
      ...
      if (isSelected || isPressed) {
        ...
        final glowPaint = Paint()
          ..color = _glowColor.withOpacity(isPressed ? 0.12 : glowIntensity * 0.08)
        ...
        final softGlowPaint = Paint()
          ..color = _glowColor.withOpacity(isPressed ? 0.15 : glowIntensity * 0.1)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 4
  ...
  Map<int, Path> _buildLocalPaths(
    ...
    Offset MT, Offset MR, Offset MB, Offset ML, Offset C,
  ) {
  ...
  void _drawHouseContent(Canvas canvas, Size size, Map<int, Path> paths) {
    ...
    // Text positions at centroid of each house shape
    final houseTextPositions = <Offset>[
      // House 1 - TOP CENTER KITE (centroid of MT, P1, C, P2)
      Offset(centerX, (top + p1y + centerY + p2y) / 4 + quarterH * 0.15),
      // House 2 - TOP RIGHT TRIANGLE (centroid of MT, TR, P2)
      Offset((centerX + right + p2x) / 3, (top + top + p2y) / 3 + quarterH * 0.1),
      // House 3 - RIGHT UPPER TRIANGLE (centroid of TR, MR, P2)
      Offset((right + right + p2x) / 3 - quarterW * 0.15, (top + centerY + p2y) / 3),
      // House 4 - RIGHT CENTER KITE (centroid of MR, P2, C, P3)
      Offset((right + p2x + centerX + p3x) / 4 + quarterW * 0.1, centerY),
      // House 5 - RIGHT LOWER TRIANGLE (centroid of MR, P3, BR)
      Offset((right + p3x + right) / 3 - quarterW * 0.15, (centerY + p3y + bottom) / 3),
      // House 6 - BOTTOM RIGHT TRIANGLE (centroid of BR, P3, MB)
      Offset((right + p3x + centerX) / 3, (bottom + p3y + bottom) / 3 - quarterH * 0.1),
      // House 7 - BOTTOM CENTER KITE (centroid of MB, P3, C, P4)
      Offset(centerX, (bottom + p3y + centerY + p4y) / 4 - quarterH * 0.15),
      // House 8 - BOTTOM LEFT TRIANGLE (centroid of MB, P4, BL)
      Offset((centerX + p4x + left) / 3, (bottom + p4y + bottom) / 3 - quarterH * 0.1),
      // House 9 - LEFT LOWER TRIANGLE (centroid of BL, P4, ML)
      Offset((left + p4x + left) / 3 + quarterW * 0.15, (bottom + p4y + centerY) / 3),
      // House 10 - LEFT CENTER KITE (centroid of ML, P4, C, P1)
      Offset((left + p4x + centerX + p1x) / 4 - quarterW * 0.1, centerY),
      // House 11 - LEFT UPPER TRIANGLE (centroid of ML, P1, TL)
      Offset((left + p1x + left) / 3 + quarterW * 0.15, (centerY + p1y + top) / 3),
      // House 12 - TOP LEFT TRIANGLE (centroid of TL, P1, MT)
      Offset((left + p1x + centerX) / 3, (top + p1y + top) / 3 + quarterH * 0.1),
    ];
    ...
    for (int i = 0; i < 12 && i < houses.length; i++) {
      ...
      final pos = houseTextPositions[i];
      ...
      final planetSize = chartSize * (isActive ? 0.042 : 0.038);  // Planets are BIGGER
      final signSize = chartSize * 0.028;  // Signs are smaller

      // Layout order (top to bottom): House Number → Planets → Sign
      
      // 1. Draw house number (tiny, at top)
      textPainter.text = TextSpan(
        ...
        style: TextStyle(
          ...
          color: isActive
              ? _glowColor
              : (isDarkMode ? Colors.white.withOpacity(0.3) : Colors.black26),
        ),
      );
      textPainter.layout();
      textPainter.paint(
        ...
        textPainter.text = TextSpan(
          ...
          style: TextStyle(
            ...
            color: isActive
                ...
                : isAscendant
                    ? _accentPrimary
                    : (isDarkMode ? Colors.white.withOpacity(0.5) : Colors.black45),
          ),
        );
        textPainter.layout();
        textPainter.paint(
...
class _HouseDetailModal extends StatelessWidget {
        ...
        Hero(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: (isAscendant ? _accentPrimary : _accentSecondary).withOpacity(0.3),
                  blurRadius: 12,
                  spreadRadius: -2,
                ),
              ],
  ...
  Widget _buildPlanetCard(String planetName) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: _getPlanetColor(planetName).withOpacity(0.2),
        ),
      ),
      child: Row(
        children: [
  ...
  Widget _buildSignificanceSection() {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            const Icon(Icons.lightbulb_outline_rounded, size: 16, color: _accentPrimary),
            const SizedBox(width: 8),
            Text(
              'House Significance',
              style: GoogleFonts.dmSans(
  ...
  Map<String, String> _getHouseSignificance(int index) {
    return {
      'title': 'House ${index + 1}',
      'description': 'Information about house ${index + 1} and the sign ${house.sign} placed here.',
    };
  }
}
